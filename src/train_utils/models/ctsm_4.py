...
class autoencoder_v3(nn.Module):
    def __init__(self, in_chn = 3, out_chn = 3, alpha = 1, tsm = True, learn = True, tsm_length=3):
        ...
        self.conv_4 = conv_block(in_chn=64, out_chn=64, alpha = alpha, tsm = tsm, learn =learn, tsm_length=self.tsm_length)
        self.conv_5 = nn.Conv2d(in_channels=64, out_channels=16, kernel_size=3, stride=1, padding=1)
        ...
        self.up_4 = up_block(in_chn=64, out_chn = 64, alpha=alpha, tsm = tsm, learn=learn, tsm_length=self.tsm_length)
        self.up_5 = up_block(in_chn=64, out_chn = self.out_chn, alpha=alpha, tsm_length=self.tsm_length)

    def forward(self, x):
        '''
        Model of CTSM for depth = 4.
        '''
